...
import net.mcreator.stevefabric.entity.render.BaseEntityRenderer;
...
public class ClientInit implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		ManegerWriterCommand.mangerWriterClient();

		for(String name : StevefabricMod.carecters ){
			BaseEntityRenderer.clientInit(name);
		}

		VariablesCustomCode.makeNewCharecterClient();
		
		ScreenRegistry.register(StevefabricMod.SteveGuiScreenType, SteveGuiGuiWindow::new);
		ScreenRegistry.register(StevefabricMod.ManegScreenType, ManegGuiWindow::new);

		ClientTickEvents.END_CLIENT_TICK.register((client) -> {
			client.execute(()->{
				ManegerCustomCode.clientManeger(client.world);
			});
		});
		ClientTickEvents.END_CLIENT_TICK.register((client) -> {
