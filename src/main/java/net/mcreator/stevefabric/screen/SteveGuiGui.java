...
public class SteveGuiGui {
	...
	public static class GuiContainerMod extends ScreenHandler  {
		public World world;
		...
		public GuiContainerMod(int id, PlayerInventory inv, PacketByteBuf data) {
			this(id, inv, new SimpleInventory(2) , new ArrayPropertyDelegate(1));
			BlockPos pos;
			if (data != null) {
		...
		public GuiContainerMod(int id, PlayerInventory inv , Inventory inventory , PropertyDelegate propertyDelegate) {
			...
			int si;
			int sj;
			this.addSlot(new Slot(this.inventory, 0, 17, 37));
			for (si = 0; si < 3; ++si)
				for (sj = 0; sj < 9; ++sj)
		...
		public void close(PlayerEntity playerIn) {
			...
			if(this.world.isClient  ){
                BaseEntity baseEntity = (BaseEntity) this.world.getEntityById(this.propertyDelegate.get(0));
                for(int i = 0 ; i<this.inventory.size();i++)
					baseEntity.inventory.setStack(i , this.inventory.getStack(i));
            }
